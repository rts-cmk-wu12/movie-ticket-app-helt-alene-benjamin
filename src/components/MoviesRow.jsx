...
export default function MoviesRow({data, movieWidth = "1000px", givenClass = "", gap = "2rem", imgHeigth = "auto", headingFontSize = "1rem", heading = "Please give heading", flexDirection = "row", rating = null, releaseDate = null, posterPath = null,  }){
    ...
    {data?.map((element)=>{
       console.log(element)
       return(
        <MoviesThumb filmID={element?.id} releaseDate={releaseDate ?  element?.release_date: null} rating={rating ? element?.vote_average: null} headingFontSize={headingFontSize} imgHeigth={imgHeigth} givenWidth={movieWidth} img={ posterPath == null ?element?.backdrop_path:element?.poster_path} title={element?.title}></MoviesThumb>
